...
def residual_unit(data, num_filter, stride, dim_match, name):
    bn1 = mx.sym.contrib.SyncBatchNorm(data=data, fix_gamma=False, eps=eps, use_global_stats=use_global_stats, ndev=2, key=name + 'bn1', name=name + '_bn1')
    act1 = mx.sym.Activation(data=bn1, act_type='relu', name=name + '_relu1')
    conv1 = mx.sym.Convolution(data=act1, num_filter=int(num_filter * 0.25), kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                               no_bias=True, workspace=workspace, name=name + '_conv1')
    bn2 = mx.sym.contrib.SyncBatchNorm(data=conv1, fix_gamma=False, eps=eps, use_global_stats=use_global_stats, ndev=2, key=name + 'bn2', name=name + '_bn2')
    act2 = mx.sym.Activation(data=bn2, act_type='relu', name=name + '_relu2')
    conv2 = mx.sym.Convolution(data=act2, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=stride, pad=(1, 1),
                               no_bias=True, workspace=workspace, name=name + '_conv2')
    bn3 = mx.sym.contrib.SyncBatchNorm(data=conv2, fix_gamma=False, eps=eps, use_global_stats=use_global_stats, ndev=2, key=name + 'bn3', name=name + '_bn3')
...
def get_resnet_feature(data, units, filter_list):
    # res1
    data_bn = mx.sym.contrib.SyncBatchNorm(data=data, fix_gamma=True, eps=eps, use_global_stats=use_global_stats, ndev=2, key='data_bn', name='bn_data')
    # data_bn = mx.sym.BatchNorm(data=data, fix_gamma=True, eps=eps, use_global_stats=use_global_stats, name='bn_sync_data')
    conv0 = mx.sym.Convolution(data=data_bn, num_filter=64, kernel=(7, 7), stride=(2, 2), pad=(3, 3),
                               no_bias=True, name="conv0", workspace=workspace)
    bn0 = mx.sym.contrib.SyncBatchNorm(data=conv0, fix_gamma=False, eps=eps, use_global_stats=use_global_stats, key='bn0', ndev=2, name='bn0')
...
def get_resnet_top_feature(data, units, filter_list):
    ...
    bn1 = mx.sym.contrib.SyncBatchNorm(data=unit, fix_gamma=False, eps=eps, use_global_stats=use_global_stats, ndev=2, key='bn1', name='bn1')
...
# data = mx.symbol.Variable(name="data")
# conv_feat = get_resnet_feature(data=data, units=units, filter_list=filter_list)
# in_shape, out_shape, a = conv_feat.get_internals()['conv0_output'].infer_shape(data=(1,3,800,1000))
#
# import pprint
# pprint.pprint(in_shape)
# pprint.pprint("8******")
